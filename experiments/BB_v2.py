...
# V18 LAP ATTACK tuning.
# Full-run log showed the largest time loss at 0.34 m/s sharp corners and
# 0.18/0.38 m/s obstacle-avoidance segments.
SPEED_ONE_LINE = 0.42

# Corner
SPEED_CORNER      = 0.36
SPEED_CORNER_HOLD = 0.32

# Acceleration / braking
SPEED_RAMP_UP   = 0.18
SPEED_RAMP_DOWN = 0.10
...
# ------------------------------------------------------------
# V16 HYBRID MULTI-POINT ROUTE
# ------------------------------------------------------------
# Lane perception itself stays the proven V15 method:
#   white LEFT/RIGHT boundary fit -> center calculation.
# The difference is that the center is represented as a sequence of points,
# similar to the old Contest_detect_v5 `mids` idea.  Avoidance then shifts
# this whole route along each point's LOCAL normal instead of moving only
# two global BEV-x targets.
ROUTE_Y_RATIOS = (0.82, 0.76, 0.70, 0.64, 0.58, 0.52, 0.48, 0.44)
ROUTE_MIN_POINTS = 3
ROUTE_CORNER_RESAMPLE_POINTS = 10
# The 0.52 sample is index 5 in ROUTE_Y_RATIOS and reproduces the old FAR
# steering geometry on normal white-lane driving.
ROUTE_FAR_CONTROL_INDEX = 5
...
# V15 speed-deadlock fix:
# During SHIFT_OUT/PASS the vehicle must keep moving so steering can actually
# create lateral displacement. Crawl speeds below ~0.1 m/s caused long stalls.
# V18 close-range speed policy.
RAW_SHIELD_CRAWL_SPEED = 0.22
RAW_SHIELD_ESCAPE_CRAWL_SPEED = 0.22

# V19 clearance-aware side-pass policy.
# Successful runs showed that 0.30 m/s is useful only when actual lateral
# clearance is already sufficient. Bearing angle alone is not a safety proof.
RAW_SHIELD_TIGHT_CLEARANCE_M = 0.15
RAW_SHIELD_FAST_CLEARANCE_M = 0.18
RAW_SHIELD_MID_CRAWL_SPEED = 0.25
RAW_SHIELD_SIDE_CRAWL_SPEED = 0.30
RAW_SHIELD_SIDE_ANGLE_DEG = 50.0

# Full stop is reserved for truly immediate footprint overlap only.
RAW_SHIELD_ABSOLUTE_STOP_X_M = 0.10
# A cone that is already far to the side must not trigger a hard stop.
# Hard-stop is only valid inside this front-facing bearing window.
RAW_SHIELD_ABSOLUTE_STOP_ANGLE_DEG = 45.0

# Final minimum speed floor while an avoidance manoeuvre is still active.
# This floor is bypassed only by the absolute-emergency condition above.
AVOID_ESCAPE_MIN_SPEED = 0.22
...
# V19: after the cone is first confirmed as passed, keep the full avoidance
# offset for a short time before returning toward the lane center.
# This gives the non-holonomic vehicle additional forward clearance.
AVOID_POST_PASS_HOLD_SEC = 0.30

# First part of return is deliberately slower; RECOVER then rises to 0.48 m/s.
AVOID_SHIFT_IN_INITIAL_SPEED = 0.38
AVOID_RECOVER_CRUISE_SPEED = 0.48

# V9.3:
# 회피 자체 때문에 0.18~0.28 m/s까지 떨어지지 않도록
# active avoidance 전체에서 기존 빠른 주행 속도 0.65 m/s를 유지한다.
# NORMAL 직선에서는 SPEED_MAX=0.75까지 허용한다.
AVOID_CRUISE_SPEED = 0.72
...
# Dynamic corner-avoidance speed: stay faster while the cone is far away,
# then reduce only when it actually approaches the vehicle.
AVOID_CORNER_FAR_X_M = 0.95
AVOID_CORNER_NEAR_X_M = 0.55
AVOID_CORNER_FAR_SPEED = 0.52
AVOID_CORNER_MID_SPEED = 0.46
AVOID_CORNER_NEAR_SPEED = 0.38
AVOID_CORNER_RECOVERY_SPEED = 0.48
...
def raw_collision_shield_speed_cap(cluster, allow_escape_crawl=False):
    """
    V15 last-resort longitudinal safety cap.

    Policy:
    - 0 m/s is allowed ONLY for truly immediate overlap:
        cx <= RAW_SHIELD_ABSOLUTE_STOP_X_M
        AND obstacle edge overlaps the hard vehicle corridor.
    - Otherwise, even hard-corridor overlap keeps a crawl speed so steering can
      create lateral displacement and finish SHIFT_OUT/PASS.
    - ``allow_escape_crawl`` is kept for call-site compatibility; V15 no longer
      needs it to decide whether a non-immediate obstacle may keep moving.
    """
    if cluster is None:
        return None

    cx = float(cluster.get("cx", 999.0))
    angle_abs = abs(float(cluster.get("angle", 180.0)))
    lateral_clearance = _raw_cluster_lateral_clearance(cluster)

    # Absolute emergency only: very close + actual footprint overlap.
    if (
        0.0 < cx <= RAW_SHIELD_ABSOLUTE_STOP_X_M
        and lateral_clearance <= RAW_SHIELD_HARD_HALF_WIDTH_M
        and angle_abs <= RAW_SHIELD_ABSOLUTE_STOP_ANGLE_DEG
    ):
        return 0.0

    # V19 clearance-aware near/side pass.
    #
    #   clearance < 0.15 m            -> 0.22 m/s
    #   0.15 <= clearance < 0.18 m    -> 0.25 m/s
    #   clearance >= 0.18 m AND
    #   angle >= 50 deg                -> 0.30 m/s
    #
    # Once the cone is genuinely outside the soft corridor and already beside
    # the vehicle, the shield can release entirely.
    if 0.0 < cx <= RAW_SHIELD_BRAKE_X_M:
        if lateral_clearance < RAW_SHIELD_TIGHT_CLEARANCE_M:
            return RAW_SHIELD_ESCAPE_CRAWL_SPEED

        if lateral_clearance < RAW_SHIELD_FAST_CLEARANCE_M:
            return RAW_SHIELD_MID_CRAWL_SPEED

        if lateral_clearance <= RAW_SHIELD_SOFT_HALF_WIDTH_M:
            if angle_abs >= RAW_SHIELD_SIDE_ANGLE_DEG:
                return RAW_SHIELD_SIDE_CRAWL_SPEED
...
def corner_collision_shield_speed_cap(
    ...
    """
    V15 corner speed cap.

    Road-guard failure (e.g. BASE_NOT_SAFE) is NOT a collision proof and must
    not stop the vehicle. It may request a crawl speed, while the raw LiDAR
    shield alone owns the true absolute-stop decision.
    """
    ...
    # BASE_NOT_SAFE / weak guard => slow down, but never command 0 m/s here.
    if (
...
class ConeAvoidanceFSM:
    ...
    def __init__(self):
        ...
        # V19 post-pass clearance hold.
        self.pass_hold_until = 0.0
        self.pass_hold_active = False
    ...
    def _start_pass_hold(self):
        if not self.pass_hold_active:
            self.pass_hold_active = True
            self.pass_hold_until = (
                time.monotonic() + AVOID_POST_PASS_HOLD_SEC
            )

    def _clear_pass_hold(self):
        self.pass_hold_active = False
        self.pass_hold_until = 0.0
    ...
    def update_observation(
        ...
        if self.state == "NORMAL":
            ...
            if state == "PASSED":
                # Unexpectedly early PASSED: keep full avoidance geometry
                # first, then apply the same clearance hold as PASS.
                self.state = "PASS"
                self._start_pass_hold()
            ...
            # treat it as a new/reacquired obstacle.
            if (
            ...
            # V19: detector PASSED or a slightly-behind cone is only the START
            # of clearance confirmation. Stay in PASS with full offset for
            # AVOID_POST_PASS_HOLD_SEC before returning to lane center.
            passed_geometry = bool(
                state == "PASSED"
                or (
                    cone is not None
                    and float(cone.get("cx", 999.0)) < -0.03
                )
            )
            ...
            if self.pass_hold_active:
                if time.monotonic() >= self.pass_hold_until:
                    self._clear_pass_hold()
                    self.state = "SHIFT_IN"
...
def build_white_center_route(det, h, w):
    """
    Build a near->far multi-point route from the CURRENT fitted white
    boundaries.  This keeps the current LaneTracker as the perception source;
    it does not return to raw contour averaging from Contest_detect_v5.
    """
    points = []
    for ratio in ROUTE_Y_RATIOS:
        y = int(round(h * float(ratio)))
        x = lane_center_at_y(det, y)
        if x is None:
            continue
        if -0.15 * w <= float(x) <= 1.15 * w:
            points.append((float(x), float(y)))
    return points


def resample_route_path(path, count=ROUTE_CORNER_RESAMPLE_POINTS):
    """Arc-length resample a near->far polyline to a stable point count."""
    if path is None or len(path) == 0:
        return []
    pts = np.asarray([(float(p[0]), float(p[1])) for p in path], dtype=np.float32)
    if len(pts) == 1:
    ...
    n = max(2, int(count))
    samples = np.linspace(0.0, total, n, dtype=np.float32)
    out = []
    j = 0
    for q in samples:
        while j + 1 < len(s) and s[j + 1] < q:
            j += 1
        if j + 1 >= len(s):
            p = pts[-1]
        else:
            ds = float(s[j + 1] - s[j])
            t = 0.0 if ds < 1e-6 else float((q - s[j]) / ds)
            p = pts[j] * (1.0 - t) + pts[j + 1] * t
        out.append((float(p[0]), float(p[1])))
    return out


def _route_local_right_normal(route_points, i):
    """
    Unit path-right normal in BEV image coordinates.
    Route order must be near -> far.
    For straight ahead tangent=(0,-1), right normal=(+1,0).
    """
    n = len(route_points)
    if n < 2:
    ...
    if i <= 0:
        p0 = np.asarray(route_points[0], dtype=np.float32)
        p1 = np.asarray(route_points[1], dtype=np.float32)
    elif i >= n - 1:
        p0 = np.asarray(route_points[n - 2], dtype=np.float32)
        p1 = np.asarray(route_points[n - 1], dtype=np.float32)
    else:
        p0 = np.asarray(route_points[i - 1], dtype=np.float32)
        p1 = np.asarray(route_points[i + 1], dtype=np.float32)
    ...
    tangent /= norm
    tx, ty = float(tangent[0]), float(tangent[1])
    return np.array([-ty, tx], dtype=np.float32)


def shift_route_points_local(route_points, near_shift_px, far_shift_px):
    """
    Shift the ENTIRE route along local path normals.

    near_shift_px / far_shift_px follow the existing convention:
      positive = path-relative RIGHT
      negative = path-relative LEFT

    For the white route, far shift reaches its full value at the historical
    FAR control point (0.52H), and stays there for farther preview points.
    For resampled corner routes the same profile is spread across the path.
    """
    ...
    n = len(pts)
    if n == 1:
        normal = _route_local_right_normal(pts, 0)
        p = np.asarray(pts[0], dtype=np.float32) + normal * float(near_shift_px)
        return [(float(p[0]), float(p[1]))]

    # White route uses a known far-control index; corner route distributes
    # the blend over about 70% of its arc and holds the far shift afterward.
    if n == len(ROUTE_Y_RATIOS):
        far_i = min(ROUTE_FAR_CONTROL_INDEX, n - 1)
    else:
        far_i = max(1, int(round((n - 1) * 0.70)))

    shifted = []
    for i, p_xy in enumerate(pts):
        t = min(1.0, float(i) / float(max(far_i, 1)))
        shift = (1.0 - t) * float(near_shift_px) + t * float(far_shift_px)
        normal = _route_local_right_normal(pts, i)
        p = np.asarray(p_xy, dtype=np.float32) + normal * shift
        shifted.append((float(p[0]), float(p[1])))
    return shifted


def steering_from_route_points(route_points, w, h):
    """
    Same lateral+heading controller as steering_from_lane(), but the control
    points are taken from a multi-point route.  On a normal white route this
    intentionally reproduces the old NEAR(0.82H)/FAR(0.52H) geometry.
    """
    if route_points is None or len(route_points) < 2:
        return None, None, None

    near = route_points[0]
    if len(route_points) == len(ROUTE_Y_RATIOS):
        far_i = min(ROUTE_FAR_CONTROL_INDEX, len(route_points) - 1)
    else:
        far_i = max(1, int(round((len(route_points) - 1) * 0.65)))
    far = route_points[far_i]

    center_near = float(near[0])
    center_far = float(far[0])

    lateral_error = (center_near - w / 2.0) / (w / 2.0)
    heading_error = (center_far - center_near) / (w / 2.0)

    steering = -(K_LATERAL * lateral_error + K_HEADING * heading_error)
    steering = float(np.clip(steering, -MAX_STEERING_DEG, MAX_STEERING_DEG))
    return steering, lateral_error, heading_error


def draw_multi_route(img, points, color, thickness=2, radius=4):
    if not points:
        return
...
def make_debug_panel(
    ...
    # V16 hybrid route debug:
    # orange = base center route, cyan = locally shifted avoidance route.
    draw_multi_route(
        bev_vis,
        det.get("base_route_points", []),
        (0, 165, 255),
        thickness=2,
        radius=3,
    )
    ...
    lines = [
        ...
        f"route: {det.get('route_source', 'WHITE')} pts={len(det.get('base_route_points', []))}",
        f"mode: {det['mode']}",
...
def format_lap_time(seconds):
    ...
    sec = seconds - minutes * 60.0
    if minutes > 0:
        return f"{minutes:d}:{sec:06.3f}"
    return f"{sec:.3f}s"
...
def main():
    ...
    # Starts after GREEN confirmation + camera return.
    lap_start_time = None
    ...
    print("=" * 72)
    print("AMET V19 CLEARANCE HOLD + HYBRID MULTI-POINT ROUTE + FRENET AVOID")
    ...
    print("Lane perception = fitted white boundaries; route = multi-point center path")
    ...
    try:
        ...
        # Start lap timing exactly when the racing control loop takes over.
        ...
        while True:
            ...
            # ------------------------------------------------
            # V16 multi-point WHITE route
            # ------------------------------------------------
            # Perception remains LEFT/RIGHT polynomial fitting.  We only turn
            # the fitted lane center into a `mids`-like near->far point path.
            white_route_points = build_white_center_route(det, h, w)
            det["white_route_points"] = white_route_points
            det["base_route_points"] = white_route_points
            det["avoid_route_points"] = []
            det["route_source"] = "WHITE"
            ...
            corner_route_points = resample_route_path(
                corner_path,
                ROUTE_CORNER_RESAMPLE_POINTS,
            )
            ...
            det["corner_route_points"] = corner_route_points
            ...
            # V16 straight / mild-curve avoidance:
            # shift the WHOLE fitted white-center route along local normals.
            # On a straight this is equivalent to the old x-offset; on a bend
            # the lateral direction rotates with the path.
            avoid_white_route_points = shift_route_points_local(
                white_route_points,
                avoid_near_px,
                avoid_far_px,
            )
            avoid_lane_steering, _, _ = steering_from_route_points(
                avoid_white_route_points,
                w,
                h,
            )
            ...
            if (
                ...
                if (
                    ...
                    and active_corner_cone is not None
                    ...
                    # Build BOTH candidates in the local path-normal frame.
                    # sign -1 = path-relative LEFT, +1 = path-relative RIGHT.
                    left_target, left_scale, left_state = (
                        shifted_corner_target_with_road_guard(
                            corner_path, w, h, -shift_mag, road_guard_mask,
                            lane_width_for_avoid,
                        )
                    )
                    right_target, right_scale, right_state = (
                        shifted_corner_target_with_road_guard(
                            corner_path, w, h, +shift_mag, road_guard_mask,
                            lane_width_for_avoid,
                        )
                    ...
                    det["corner_left_scale"] = float(left_scale)
                    det["corner_right_scale"] = float(right_scale)
                    ...
                    def _candidate_score(sign, scale):
                        score = float(scale)
                        if sign == preferred_sign:
                            score += CORNER_DUALPATH_PREFERENCE_BONUS
                        if sign == float(avoidance.offset_sign):
                            score += CORNER_DUALPATH_CONTINUITY_BONUS
                        if float(scale) < CORNER_DUALPATH_MIN_SCALE:
                            score -= 0.50
                        return score

                    left_score = _candidate_score(-1.0, left_scale)
                    right_score = _candidate_score(+1.0, right_scale)

                    if left_score >= right_score:
                        chosen_sign = -1.0
                        avoid_corner_target = left_target
                        avoid_corner_shift_scale = float(left_scale)
                        avoid_corner_guard_state = left_state
                        avoidance.avoid_direction = "LEFT"
                        det["corner_plan_side"] = "LEFT"
                    else:
                        chosen_sign = +1.0
                        avoid_corner_target = right_target
                        avoid_corner_shift_scale = float(right_scale)
                        avoid_corner_guard_state = right_state
                    ...
                    # V16: the road guard still decides side/accepted scale,
                    # but steering is no longer based on ONE shifted point.
                    # Shift the complete yellow route along its local normals.
                    if avoidance.state == "SHIFT_OUT":
                        near_ratio_cmd = min(
                            AVOID_SHIFT_NEAR_RATIO,
                            shift_ratio,
                        )
                        far_ratio_cmd = shift_ratio
                    else:  # PASS
                        near_ratio_cmd = shift_ratio
                        far_ratio_cmd = shift_ratio

                    corner_near_shift_px = (
                        chosen_sign
                        * near_ratio_cmd
                        * float(lane_width_for_avoid)
                        * float(avoid_corner_shift_scale)
                    )
                    corner_far_shift_px = (
                        chosen_sign
                        * far_ratio_cmd
                    ...
                    avoid_corner_route_points = shift_route_points_local(
                        corner_route_points,
                        corner_near_shift_px,
                        corner_far_shift_px,
                    )
                    avoid_corner_target = choose_corner_target(
                        avoid_corner_route_points,
                        w,
                        h,
                    )
                    ...
                    avoid_corner_route_points = corner_route_points
            ...
            # Unified route representation used for debugging and avoidance.
            if corner_active and corner_route_points:
                det["base_route_points"] = corner_route_points
                det["route_source"] = "YELLOW"
                if avoid_corner_route_points:
                    det["avoid_route_points"] = avoid_corner_route_points
            else:
                det["base_route_points"] = white_route_points
                det["route_source"] = "WHITE"
                if avoidance.state != "NORMAL":
                    det["avoid_route_points"] = avoid_white_route_points
            ...
            det["pass_hold_active"] = bool(avoidance.pass_hold_active)
            det["pass_hold_remaining"] = max(
                0.0,
                float(avoidance.pass_hold_until - time.monotonic())
            ) if avoidance.pass_hold_active else 0.0
            ...
            if (
                ...
                if corner_active:
                    # V18: do not force every curved avoidance segment to
                    # 0.38 m/s from the moment a distant cone is detected.
                    cone_x_for_speed = None
                    if (
                        latest_cone_result is not None
                        and latest_cone_result.get("cluster") is not None
                    ):
                        cone_x_for_speed = float(
                            latest_cone_result["cluster"].get("cx", 999.0)
                        )

                    if avoidance.state == "SHIFT_IN":
                        corner_avoid_speed = AVOID_SHIFT_IN_INITIAL_SPEED
                    elif avoidance.state == "RECOVER":
                        corner_avoid_speed = AVOID_RECOVER_CRUISE_SPEED
                    elif (
                        cone_x_for_speed is None
                        or cone_x_for_speed > AVOID_CORNER_FAR_X_M
                    ):
                        corner_avoid_speed = AVOID_CORNER_FAR_SPEED
                    elif cone_x_for_speed > AVOID_CORNER_NEAR_X_M:
                        corner_avoid_speed = AVOID_CORNER_MID_SPEED
                    else:
                        corner_avoid_speed = AVOID_CORNER_NEAR_SPEED

                    target_speed = min(
                        AVOID_CRUISE_SPEED,
                        corner_avoid_speed,
                    )
                    det["corner_avoid_speed_cmd"] = corner_avoid_speed
                else:
                    if avoidance.state == "SHIFT_IN":
                        target_speed = AVOID_SHIFT_IN_INITIAL_SPEED
                    elif avoidance.state == "RECOVER":
                        target_speed = AVOID_RECOVER_CRUISE_SPEED
                    else:
                        target_speed = AVOID_CRUISE_SPEED
                    det["corner_avoid_speed_cmd"] = None
            ...
            if latest_raw_collision_cluster is not None:
                ...
                raw_angle_abs = abs(float(
                    latest_raw_collision_cluster.get("angle", 180.0)
                ))
                raw_cy = float(
            ...
            det["raw_collision_angle_abs"] = raw_angle_abs
            det["raw_escape_crawl"] = raw_escape_crawl
            if raw_shield_cap is not None:
                target_speed = min(target_speed, raw_shield_cap)

            # ------------------------------------------------
            # V15 avoidance deadlock guard
            # ------------------------------------------------
            # SHIFT_OUT/PASS require forward motion; otherwise steering cannot
            # generate lateral displacement. Keep a minimum escape speed unless
            # the raw LiDAR geometry says an obstacle is within the absolute
            # emergency distance AND actually overlaps the vehicle corridor.
            absolute_emergency = False
            if latest_raw_collision_cluster is not None:
                raw_x = float(
                    latest_raw_collision_cluster.get("cx", 999.0)
                )
                raw_clear_for_stop = _raw_cluster_lateral_clearance(
                    latest_raw_collision_cluster
                )
                raw_angle_for_stop = abs(float(
                    latest_raw_collision_cluster.get("angle", 180.0)
                ))
                absolute_emergency = bool(
                    0.0 < raw_x <= RAW_SHIELD_ABSOLUTE_STOP_X_M
                    and raw_clear_for_stop <= RAW_SHIELD_HARD_HALF_WIDTH_M
                    and raw_angle_for_stop <= RAW_SHIELD_ABSOLUTE_STOP_ANGLE_DEG
                )

            avoid_escape_active = bool(
                avoidance.state in ("SHIFT_OUT", "PASS")
                and control_steering is not None
            )

            if avoid_escape_active and not absolute_emergency:
                target_speed = max(
                    target_speed,
                    AVOID_ESCAPE_MIN_SPEED,
                )

            det["avoid_absolute_emergency"] = absolute_emergency
            det["avoid_escape_floor_active"] = (
                avoid_escape_active and not absolute_emergency
            )
            ...
            lap_elapsed = (
                time.perf_counter() - lap_start_time
                if lap_start_time is not None
                else 0.0
            )

            status = (
                f"RACING {format_lap_time(lap_elapsed)}  "
                f"ctrl={control_mode}  "
                f"route={det.get('route_source', 'WHITE')}/{len(det.get('base_route_points', []))}  "
                f"mode={det['mode']}  "
            ...
            if now - last_print >= PRINT_INTERVAL:
                print(
                    ...
                    f"route={det.get('route_source', 'WHITE'):<6}/{len(det.get('base_route_points', [])):<2} "
                    f"mode={det['mode']:<10} "
                    f"conf={det['confidence']:.2f} "
                    ...
                    f"lap={format_lap_time(lap_elapsed)} "
                    ...
                    f"rawang={det.get('raw_collision_angle_abs')} "
                    f"absstop={int(bool(det.get('avoid_absolute_emergency', False)))} "
                    f"hold={det.get('pass_hold_remaining', 0.0):.2f}s "
                    f"escape={int(bool(det.get('raw_escape_crawl', False)))} "
                    f"offN={avoid_near_ratio:+.2f} "
                    f"offF={avoid_far_ratio:+.2f}"
                )
        ...
        # Freeze timing before cleanup latency is added.
        lap_elapsed_final = None
        if lap_start_time is not None:
            lap_elapsed_final = time.perf_counter() - lap_start_time
        ...
        if lap_elapsed_final is not None:
            print("=" * 72)
            print(
                f"[LAP] FINAL ({stop_reason}) = "
                f"{format_lap_time(lap_elapsed_final)}"
            )
            print("=" * 72)
        else:
            print("[LAP] timer was not started (race gate not cleared)")
