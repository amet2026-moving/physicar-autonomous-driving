...
# 직선 구간 최고 속도 (m/s). 0.75 -> 2.0으로 올림(실차 요청) -- 이만큼 빨라지면
# 장애물까지 도달하는 시간이 그만큼 짧아지므로, 회피 반응거리/EMA 속도도 같이
# 올려야 한다(RAW_SHIELD_BRAKE_X_M, AVOID_OFFSET_ALPHA, decision_params.
...
AVOID_CRUISE_SPEED = 0.90    # 회피 중일 때(아직 크게 안 피한 상태) 속도 상한 (m/s) -- SPEED_MAX(2.0)보다는 낮게 유지해 회피 중엔 항상 감속되게 함
...
# 속도로 달리면 1초가 채 안 돼 통과해버리는 거리라, EMA가 채 오르기도 전에 장애물을
# 지나쳐 "회피가 늦고 각도가 작다"는 문제의 핵심 원인이었다. SPEED_MAX를 2.0으로
# 올린 지금은 더더욱 짧으므로 1.30m로 늘림 (LIDAR_CLUSTER_MAX_RANGE_M=2.0m 이내).
RAW_SHIELD_BRAKE_X_M = 1.30
