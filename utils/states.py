...
class ObstacleState(Enum):
    """카메라+라이다 융합 결과로 판단한 장애물 상태.
    회피 방향은 항상 오른쪽으로 고정(팀 결정: 왼쪽 흰선~노란선 코리도를 기본 주행선으로
    쓰고, 장애물을 만나면 무조건 오른쪽으로 피했다가 돌아온다)이라 좌/우를 가릴 필요가
    없다 -- "지금 내 주행 코리도를 막고 있는가"만 본다."""
    CLEAR = "CLEAR"      # 코리도 안에 반응할 만큼 가까운 장애물 없음
    BLOCKED = "BLOCKED"   # 코리도 안 전방에 장애물 있음 -- 오른쪽으로 회피
