"""LiDAR 기반 장애물 회피 - v4_ynz_1line_match.

이 모듈은 단일 차선 추종 제어기(lane/v4_ynz_1line.py)와 최적의 시너지를 내도록 설계된 
장애물 회피 코드입니다.

주요 설계 특징:
1. 동적 차선 스위칭 제어:
   - 평소(장애물 없을 때)에는 config.target_lane = "LEFT"로 설정하여 왼쪽 차선 주행을 강제합니다.
   - 전방/왼쪽 차선 영역에 장애물이 감지되는 즉시 config.target_lane = "RIGHT"로 전환하여 
     라인트레이서가 스스로 오른쪽 차선으로 변경하도록 차선 목표를 바꿉니다.
   - 장애물이 완전히 사라지고 2초 동안 감지되지 않으면 다시 config.target_lane = "LEFT"로 복귀합니다.
2. 강력하고 신속한 회피 조향:
   - "장애물이 너무 커서 피해도 닿음" 문제를 해결하기 위해 안전 마진(SAFETY_MARGIN)을 0.20m로 넓히고, 
     조향 이탈 게인을 대폭 높여(config.OBSTACLE_STEER_GAIN / 20.0) 더 급격하고 넓게 휘어지도록 제어합니다.
3. 1프레임 즉시 진입 및 40도 최대 바이어스 적용.
"""
import time
import math
import config
...
class ObstacleAvoider:
    def __init__(self):
        ...
        # v4_ynz_1line 매칭 튜닝 파라미터 (장애물 우회 우선)
        self.VEHICLE_HALF_WIDTH = 0.15  # 차량 물리적 반폭
        self.SAFETY_MARGIN = 0.20       # 큰 장애물 우회를 위해 안전 여유를 0.20m로 대폭 확장
        ...
        # 회피 조향 및 속도 제어 강도 보정
        self.OBSTACLE_BIAS_ALPHA = 0.9    # 회피 조향 반응 지연 최소화
        self.SLOWDOWN_EXTRA_MARGIN = 0.40 # 감속 시점을 당기기 위한 보정값 (m)
        
        # 1line 매칭용 추가 보정치
        self.LEFT_GAP_BONUS = 40.0        # 왼쪽 gap 선택에 부여하는 기본 보너스
    ...
    def _process_lidar(self, points):
        ...
        for i in range(181):
            if grid_observed[i] and grid_dist[i] < config.OBSTACLE_MAX_RANGE:
                d = grid_dist[i]
                # 차량 안전폭(0.70m) 기준 inflation 각도 계산
    ...
    def _score_gaps(self, gaps, curve_direction):
        ...
        기본 주행이 왼쪽 차선(v4_ynz_1line)이므로 왼쪽 방향 gap에 가중치를 추가로 배정합니다.
        """
        scored_gaps = []

        # curve_direction 방향에 따라 목표 조향 각 설정 (+:좌 / -:우)
        # 1line 매칭을 위해 기본 타겟 각도를 왼쪽(+25.0도)으로 배치하여 기본 차선 주행을 유도합니다.
        if curve_direction == 1.0:
            theta_target = 35.0
        elif curve_direction == -1.0:
            theta_target = -30.0
        else:
            theta_target = 25.0  # 기본 왼쪽 차선 주행 경향에 최적화
        ...
        for gap in gaps:
            ...
            # Alignment Penalty (가중치 50.0으로 제한)
            ...
            # 1line 매칭용 왼쪽 gap(center_angle >= 0) 가산 보너스 [+40]
    ...
    def step(self, points, curve_direction, corner_active, now):
        ...
        # 초반에 아무것도 안보일 때 급정지/후진하는 대기 에러 방지
        # front_points_count가 부족하더라도 1초 이내이면 기동 시간을 고려해 세이프 가동을 허용함
        ...
        # 전방 및 왼쪽 범위(-20도 ~ +45도) 내에 장애물이 임계범위(1.90m) 안으로 들어오면 오른쪽 차선으로 즉각 우회 명령
        ...
        if left_lane_obstacle:
            ...
            # 2초 동안 왼쪽 차선에 장애물이 검출되지 않으면 다시 원래 왼쪽 차선 복귀
            if now - self.last_obstacle_time > 2.0:
                config.target_lane = "LEFT"

        # 오른쪽 차선(오른쪽 앞 영역: -90도 ~ -10도)에 장애물이 보인 즉시 피하기 감지용 체크
        ...
        if self.state == "AVOID":
            ...
            if scored_gaps:
                ...
                # 조향 이탈 게인을 대폭 높여서 큰 장애물 회피를 보장 (config.OBSTACLE_STEER_GAIN / 20.0)
                target_bias = selected_gap["center_angle"] * intensity * (config.OBSTACLE_STEER_GAIN / 20.0)
                
                # 최대 복원각 증폭 (40.0도)
                obstacle_bias_max = 2.0 * config.STEER_MAX  # 40.0 도
                target_bias = max(-obstacle_bias_max, min(obstacle_bias_max, target_bias))
            else:
                self.obstacle_direction = 0.0
                self.state = "STOPPED"
                return {
