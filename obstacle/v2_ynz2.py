"""LiDAR 기반 장애물 회피 - v3_ynz_avoid_priority.

이 모듈은 장애물이 경로 중앙에 있을 때 차량이 밀고 가는 문제를 해결하기 위해 
회피성능을 극대화한 "두 번째" 개량 버전입니다.

주요 변경점:
1. 즉각적인 회피 반응 (AVOID_ENTER_FRAMES = 1)
2. 높은 조향 편향 한계 (OBSTACLE_BIAS_MAX = 40.0)로 라인트레이서의 반대 조향 복귀 제어 압도
3. 조향 반응 스무딩 지연 최소화 (OBSTACLE_BIAS_ALPHA = 0.9)
4. 장애물 탐색 시 정렬 가중치 페널티를 낮춰 차선보다 회피를 우선시함
5. 감속 시작 시점을 당기기 위해 min_dist 값을 강제 보정하여 전방 감속 유도
"""
import time
import math
import config
...
class ObstacleAvoider:
    def __init__(self):
        ...
        self.SAFETY_MARGIN = 0.15       # 회피 우선을 위해 안전 여유를 0.15m로 확장
        self.REQUIRED_GAP_WIDTH = 2 * (self.VEHICLE_HALF_WIDTH + self.SAFETY_MARGIN)  # 0.60m
        ...
        self.AVOID_ENTER_FRAMES = 1      # 장애물 발견 시 즉각 진입 (기존 3에서 1로 단축)
        ...
        # 회피 조향 및 속도 제어 강도 보정
        self.OBSTACLE_BIAS_ALPHA = 0.9    # 회피 조향 반응 지연 최소화 (기존 0.5에서 0.9로 변경)
        self.SLOWDOWN_EXTRA_MARGIN = 0.40 # 감속 시점을 앞당기기 위해 min_dist에 감산할 보정값 (m)
    ...
    def _process_lidar(self, points):
        ...
        for i in range(181):
            if grid_observed[i] and grid_dist[i] < config.OBSTACLE_MAX_RANGE:
                d = grid_dist[i]
                # 차량 안전폭(0.60m) 기준 inflation 각도 계산
    ...
    def _score_gaps(self, gaps, curve_direction):
        ...
        회피 우선을 위해 Alignment Penalty 가중치를 낮춰 차선 유지보다 장애물 우회를 극대화합니다.
        """
        scored_gaps = []
        ...
        for gap in gaps:
            ...
            # Alignment Penalty (가중치를 50.0으로 축소하여 회피 방향보다 차선 방향을 따라가려는 성향 억제)
    ...
    def step(self, points, curve_direction, corner_active, now):
        ...
        # NORMAL -> AVOID 전이 (디바운스 반영: 장애물 발견 시 1프레임만에 즉시 진입)
        ...
        if self.state == "AVOID":
            ...
            if scored_gaps:
                ...
                # 회피 우선을 위해 조향 출력 제한치를 극대화하여 차선 트레이서의 반대 조향을 압도함
        ...
        # 감속 시점을 당기기 위해 min_dist 값을 강제 보정한 값을 main.py에 반환하여 감속 유도
        returned_min_dist = max(0.1, min_dist - self.SLOWDOWN_EXTRA_MARGIN) if has_obstacle else min_dist
